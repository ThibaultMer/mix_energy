...
def test_load_all_from_schemas_uses_standard_table_and_truncate(monkeypatch):
    calls = {"load": [], "mark": []}
...
def test_load_all_from_schemas_retries_with_string_schema(monkeypatch):
    ...
    assert calls["load"][0][1] == bigquery.WriteDisposition.WRITE_TRUNCATE
    assert calls["load"][1][1] == bigquery.WriteDisposition.WRITE_TRUNCATE
    assert calls["load"][0][2].endswith("file_b")
    assert not calls["load"][0][2].endswith("file_b_histo")
