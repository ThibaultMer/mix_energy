...
def dag_eco2mix_regional_tr():
    ...
    @task.bash(task_id="dbt_eco2mix_regional_tr_agre")
    def dbt_reg_tr_agre():
        return f"""
            cd {DBT_DIR} &&
            dbt run --select reg_tr_agre_j --target prod
            """

    @task.bash(task_id="dbt_eco2mix_regional_tr_predi")
    def dbt_reg_tr_predi():
        return f"""
            cd {DBT_DIR} &&
            dbt run --select reg_tr_predi --target prod
            """
    ...
    dbt_eco2mix_regional_tr_agre_task: Any = dbt_reg_tr_agre()
    dbt_eco2mix_regional_tr_predi_task: Any = dbt_reg_tr_predi()

    (
        check_bucket_connection_task
        >> ingest_csv_to_bucket_task
        >> transfer_csv_from_bucket_to_bigquery_task
        >> [dbt_eco2mix_regional_tr_agre_task, dbt_eco2mix_regional_tr_predi_task]
    )
