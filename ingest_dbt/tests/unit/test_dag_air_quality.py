...
def test_dag_air_quality_structure(monkeypatch):
    _install_airflow_stubs(monkeypatch)

    path = (
        Path(__file__).resolve().parents[3] / "airflow" / "dags" / "dag_air_quality.py"
    )
    spec = importlib.util.spec_from_file_location("dag_air_quality", path)
    ...
    assert dag.dag_id == "dag_air_quality"
    assert dag.schedule.crons == ("30 9 * * 1-5",)
    assert dag.schedule.timezone == "Europe/Paris"
    assert dag.tags == ["air_quality", "ingestion"]
    assert dag.task_ids == [
        "check_bucket_connection",
        "ingest_air_quality_to_bucket",
